...
class BannerTest(TestCase):
    def test_bannercontent(self):
        page = Page.objects.create(title="Home", slug="home", override_url="/")

        mediafile = MediaFile()
        mediafile.file.save("whatever.txt", ContentFile("whatever"), save=True)
        ...
        page.bannercontent_set.create(
            region="main", ordering=0, specific=banner, type="leaderboard"
        )
        ...
        self.assertContains(response, impression_url)
        ...
        self.assertContains(self.client.get(impression_url), "?")

        banner = Banner.objects.get(pk=banner.pk)
        self.assertEqual(banner.impressions, 0)

        self.assertContains(
            self.client.get(impression_url, HTTP_X_REQUESTED_WITH="XMLHttpRequest"),
            "+1",
        )
        ...
        self.assertRedirects(
            self.client.get(click_url), "http://testserver/test-banner-1/"
        )
